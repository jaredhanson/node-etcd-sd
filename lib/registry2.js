...
Registry.prototype.resolve = function(domain, type, cb) {
  ...
  client._ls(dir, function(error, children) {
    if (error) {
      return cb(error);
    }

    children = children.node.nodes || [];
    var records = []
      , idx = 0;
...
Registry.prototype.types = function(domain, cb) {
  ...
  this._ls(dir, function (err, res) {
    ...
    res = res.node ? res.node.nodes : [];
    var types = res.map(function(c) { return decodeURIComponent(c.key); });
