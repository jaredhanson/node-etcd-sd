...
Registry.prototype.unannounce = function(domain, type, uid, cb) {
  ...
  var client = this._client
    // https://github.com/coreos/etcd/issues/669 Double Encoding.
    , path = [ this._ns, domain, encodeURIComponent(encodeURIComponent(type)), uid ].join('/');
...
Registry.prototype.domains = function(cb) {
  this._ls(this._ns, function (err, res) {
    var data = res
      , results;
    if (err) {
      return cb(err);
...
Registry.prototype.resolve = function(type, cb) {
  var self = this
    , client = this._client
  ...
  client.setPath(dir, function(err) {
    if (err) {
      if (err.code != 102) {
        return cb(err);
      }
